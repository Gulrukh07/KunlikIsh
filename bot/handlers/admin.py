...
from bot.buttons.inline import for_admin, employee_response, deal_button
from bot.buttons.reply import back_button, rating
from bot.states import WorkForm
from db.models import Work, Employer, Employee, WorkStatus, Rating
...
async def send_to_workers(callback: CallbackQuery, bot: Bot) -> None:
    ...
    await bot.send_message(
        ...
        text=_("🎉 Tabriklaymiz, sizning buyurtmangiz admin tomonidan qabul qilindi\n"
               "U ishchilar tomonidan qabul qilinsa, biz sizga xabar beramiz"),
...
async def admin_response1(callback: CallbackQuery):
    ...
    await callback.bot.send_message(
        chat_id=employer.chat_id,
        text=_("☹️ Afsuski, Sizning buyurtmangiz rad etildi"),
        reply_markup=back_button()
    )
...
async def admin_response1(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    if employee_username:
        ...
        await bot.send_message(
            chat_id=employer_id,
            text=_("📌 Buyurtmangiz {phone} qabul qilindi\n"
                   "Ishchi bilan kelishgandan so'ng xabar bering⁉").format(phone=phone),
            reply_markup=deal_button(employee_id=ee_id, employer_id=er_id)
        )
    ...
    await bot.send_message(
        chat_id=admin_id,
        text=_(
            "📢 {employer_id} - sonli ish beruvchining {work_id} - sonli buyurtmasi {employee_id} - sonli ishchi tomonidan qabul qilindi"
        ).format(
...
async def deal_handler(callback: CallbackQuery, bot: Bot, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=admin_id,
        text=_("{employer_id} - sonli buyurtmachining {work_id} - sonli buyurtmasi {employee_id} - sonli ishchi bilan kelishildi").format(
